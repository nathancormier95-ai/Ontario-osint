...
async function analyzeLocalFile(file: File): Promise<FileEvidence> {
...
export function EvidenceTools() {
  ...
  function openDomainSource(kind: "icann" | "certificates") {
    const normalized = normalizeDomain(domain);
    if (!normalized) {
      toast.error("Enter a complete domain such as example.ca. This tool does not accept IP addresses or URLs with paths.");
      return;
    }
    const href = kind === "icann"
      ? `https://lookup.icann.org/en/lookup?name=${encodeURIComponent(normalized)}`
  ...
  function openVehicleReference() {
    ...
    window.open(`https://vpic.nhtsa.dot.gov/api/vehicles/DecodeVinValuesExtended/${encodeURIComponent(normalized)}?format=json`, "_blank", "noopener,noreferrer");
  ...
  async function copyHash() {
    ...
    try {
      await navigator.clipboard.writeText(fileEvidence.sha256);
