...
## Conditional scroll cue

At a short desktop height (`1280×720`), the navigation displays the compact `Scroll for more` cue while the lower section destinations are clipped. The cue is positioned beneath the scroll region and uses the existing yellow civic accent. The mobile entry point remains visible at `390×667`; its drawer navigation uses the same conditional cue when opened and clipped.

At tall desktop (`1280×1400`) and tall mobile (`390×1400`) viewports, the visible page/header remains uncluttered and no navigation cue is shown until a navigation drawer/list is actually clipped. The conditional logic therefore avoids adding noise when there is no additional hidden navigation content.
